...
/**
 * In-app YouTube mode. It does not require SYSTEM_ALERT_WINDOW.
 * The YouTube page and the Arabic subtitle layer live in the same Activity.
 * When YouTube renders captions, the page script sends caption text to the
 * native translator. This mode is intended for devices such as Redmi A3
 * where the system disables app overlays.
 */
class YouTubePlayerActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val open = Button(this).apply {
            text = "تشغيل"
            setOnClickListener { loadYouTube(url.text.toString().trim()) }
        }
        controls.addView(url)
        ...
        subtitle = TextView(this).apply {
            text = "الترجمة العربية ستظهر هنا عند توفر captions في الفيديو"
            textSize = 18f
    ...
    inner class CaptionBridge {
        ...
        fun onCaption(text: String) {
            ...
            lastCaption = clean
            LocalTranslator.translateToArabic(clean) { arabic ->
                runOnUiThread {
    ...
    companion object {
        private const val CAPTION_SCRIPT = """
            (function(){
              try {
                var nodes = document.querySelectorAll('.ytp-caption-segment, .caption-window, .ytp-caption-window-container');
                var parts=[];
                nodes.forEach(function(n){ var t=(n.innerText||n.textContent||'').trim(); if(t) parts.push(t); });
